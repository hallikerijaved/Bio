  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', async (event) => {
      ...
      if (userSubject) {
        ...
        formData.set('message', `Subject: ${userSubject}\n\n${existingMessage}`)
      }
      ...
      try {
        ...
        const serverMessage = data.message || (data.errors && data.errors[0] && data.errors[0].message)

        if (!response.ok || !data.success) {
          throw new Error(serverMessage || 'Your message could not be sent right now.')
        }
